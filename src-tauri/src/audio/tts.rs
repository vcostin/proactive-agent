//! Piper binary spawn + cpal playback — the production [`SpeakEngine`].
//!
//! Path join, voice resolve, and PlaybackGate begin live in [`super::piper_speak`].
...
use super::piper_speak::SpeakEngine;
...
/// Production engine: Piper stdin→WAV→PCM→cpal, honouring the playback gate.
#[derive(Debug, Default, Clone, Copy)]
pub struct PiperPlayEngine;

#[async_trait]
impl SpeakEngine for PiperPlayEngine {
...
fn find_piper() -> Result<std::path::PathBuf> {
    if let Some(p) = crate::find_sidecar("piper") {
        return Ok(p);
    }
...
mod tests {
    use super::*;
    use crate::audio::piper_speak::clean_for_speech;
    ...
    fn resample_upsample_doubles_length() {
        ...
        let expected = samples.len() * 2 - 1;
        assert!(
            (out.len() as i64 - expected as i64).abs() <= 1,
            "upsampled len {} ≈ expected {}",
            out.len(),
            expected
        );
    }
